...
module.exports = {
    async up(queryInterface) {
        ...
        const tableExists = await queryInterface.sequelize.query("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'users')", { type: queryInterface.sequelize.QueryTypes.SELECT });
        ...
        await queryInterface.createTable('activities', {
            ...
            action: {
                type: DataTypes.ENUM('CREATE', 'UPDATE', 'DELETE', 'APPROVE', 'REJECT', 'REASSIGN', 'SUBMIT', 'RESUBMIT', 'UPDATE_PERMISSIONS'),
                allowNull: false,
            },
            resource_type: {
                type: DataTypes.ENUM('SPACE', 'CABINET', 'RECORD', 'FILE'),
                allowNull: false,
            },
            resource_id: {
            ...
            status: {
                type: DataTypes.ENUM('completed', 'pending', 'rejected', 'default'),
                allowNull: true,
            },
            timestamp: {
                type: DataTypes.DATE,
