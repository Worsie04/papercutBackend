import { Stamp, StampType } from '../models/stamp.model';
...
export class StampService {
    ...
    async listUserStamps(userId: string): Promise<Pick<Stamp, 'id' | 'filename' | 'publicUrl' | 'stampType' | 'createdAt'>[]> {
        try {
            const stamps = await Stamp.findAll({
                where: { userId },
                attributes: ['id', 'filename', 'publicUrl', 'stampType', 'createdAt'],
                order: [['createdAt', 'DESC']]
            });
            return stamps.map(stamp => ({
                id: stamp.id,
                filename: stamp.filename,
                publicUrl: stamp.publicUrl,
                stampType: stamp.stampType,
                createdAt: stamp.createdAt
            }));
        } catch (error) {
            console.error('Error fetching user stamps:', error);
            throw new AppError(StatusCodes.INTERNAL_SERVER_ERROR, 'Failed to fetch stamps.');
        }
    }
    ...
    async uploadStamp(
        ...
        stampType: StampType = StampType.UPLOADED
    ): Promise<Stamp> {
        if (!file) {
        ...
        const storageKey = `stamps/user-${userId}/${uniqueFilename}`;
        ...
        try {
            ...
            throw new AppError(StatusCodes.INTERNAL_SERVER_ERROR, 'Failed to upload stamp to storage.');
        }

        try {
            const newStampRecord = await Stamp.create({
                userId,
            ...
            return newStampRecord;
        } catch (dbError) {
            console.error('Error saving stamp metadata to DB:', dbError);
            try {
            ...
            throw new AppError(StatusCodes.INTERNAL_SERVER_ERROR, 'Failed to save stamp information after upload.');
        }
    }

    async deleteStamp(userId: string, stampId: string): Promise<void> {
        try {
            const stampRecord = await Stamp.findOne({
                where: { id: stampId, userId: userId }
            });

            if (!stampRecord) {
                throw new AppError(StatusCodes.NOT_FOUND, 'Stamp not found or access denied.');
            }
            if (!R2_BUCKET_NAME) {
            ...
            const deleteParams = {
                Bucket: R2_BUCKET_NAME,
                Key: stampRecord.storageKey,
            };

            try {
                console.log(`Deleting stamp ${stampRecord.storageKey} from R2 bucket ${R2_BUCKET_NAME}`);
                await s3Client.send(new DeleteObjectCommand(deleteParams));
                console.log('Deletion from R2 successful.');
            } catch (storageError) {
                console.error(`Failed to delete stamp ${stampRecord.storageKey} from R2. Proceeding to delete DB record.`, storageError);
            }

            await stampRecord.destroy();
            console.log(`Stamp record ${stampId} deleted from DB.`);

        } catch (error) {
            if (error instanceof AppError) throw error;
            console.error('Error deleting stamp:', error);
            throw new AppError(StatusCodes.INTERNAL_SERVER_ERROR, 'Could not delete stamp.');
        }
    }

    async getStampById(userId: string, stampId: string): Promise<Stamp> {
        try {
            ...
            throw new AppError(StatusCodes.INTERNAL_SERVER_ERROR, 'Could not fetch stamp.');
        }
    }
}
